  ...
  async getPendingTasks(establishmentId: string): Promise<PendingTask[]> {
    ...
    for (const sector of cleaningSectors) {
      for (const subSector of sector.subSectors) {
        for (const eq of subSector.equipment) {
          for (const task of eq.tasks) {
            ...
            if (!checkedTaskIds.has(task.id)) {
              tasks.push({
                ...
                status: 'PENDING',
                title: task.name,
                subtitle: `${sector.name} — ${subSector.name} — ${eq.name}`,
                link: '/cleaning',
                taskId: task.id,
  ...
  async getTasksHistory(
    ...
    for (const check of cleaningChecks) {
      allDone.push({
        ...
        subtitle: `${check.task.equipment?.subSector?.sector?.name || ''} — ${check.task.equipment?.name || ''}`,
