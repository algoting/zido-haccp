...
export class UploadsMiddleware implements NestMiddleware {
  use(req: Request, res: Response, next: () => void) {
    ...
    // Verify JWT authentication (from header or query parameter)
    const authHeader = req.headers['authorization'];
    const queryToken = (req.query?.token as string) || undefined;
    let token = queryToken;
    if (authHeader) {
      const [type, hToken] = authHeader.split(' ');
      if (type === 'Bearer' && hToken) {
        token = hToken;
      }
    }
    ...
    try {
      ...
      const filePath = url.substring(uploadIndex + '/uploads/'.length).split('?')[0];
